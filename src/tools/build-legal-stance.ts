...
export async function buildLegalStance(
  ...
  for (const ftsQuery of queryVariants) {
    ...
    try {
      const rows = (db.prepare(sql).all(...params) as Omit<LegalStanceResult, '_citation'>[]).map(row => ({
        ...row,
  ...
  // LIKE fallback — last resort when all FTS5 variants return empty
  {
    ...
    try {
      const rows = (db.prepare(likeSql).all(...likeParams) as Omit<LegalStanceResult, '_citation'>[]).map(row => ({
        ...row,
        _citation: buildCitation(
          `${row.document_title} ${row.provision_ref}`,
          `${row.provision_ref}, ${row.document_title}`,
