#!/usr/bin/env tsx
/**
 * Database builder for Nicaragua Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Nicaragua Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Nicaraguan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'NI');
    insertMeta.run('source', 'nicaragua.justia.com');
