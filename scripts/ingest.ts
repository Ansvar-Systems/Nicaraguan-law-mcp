#!/usr/bin/env tsx
/**
 * Nicaraguan Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable law
 * from nicaragua.justia.com (HTML full text).
 *
 * Pipeline per law:
 ...
 *
 * Data source: nicaragua.justia.com
...
import { parseNILawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Nicaraguan Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('========================================================\n');
  console.log('  Source: nicaragua.justia.com');
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseNILawHtml(html, act);
  ...
  console.log(`\n  Source:      nicaragua.justia.com (HTML)`);
