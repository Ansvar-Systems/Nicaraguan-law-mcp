/**
 * Nicaraguan Law HTML Parser
 *
 * Parses law text from HTML pages on nicaragua.justia.com
 * into structured provisions.
 *
 * Nicaraguan civil law article patterns:
 *   "Artículo N."  / "Art. N"  / "ARTÍCULO N."
 *   "Arto. N"      / "ARTO. N" (Nicaragua-specific abbreviation)
 *   "Artículo Único"
 *
 * Structure patterns:
 ...
 *
 * HTML-only pipeline -- no PDF extraction, no child_process.
 */
...
/**
 * Nicaraguan article heading patterns.
 *
 * Standard forms:
 ...
 *
 * Nicaragua-specific:
 *   Arto. 1        / ARTO. 1    (common Nicaraguan abbreviation)
 *   Arto 1.        / Arto. 1.-
 */
const ARTICLE_PATTERNS = [
  // Standard + Nicaragua "Arto." abbreviation
  /(?:^|\n)\s*(?:ART[ÍI]CULO|Art[íi]culo|ARTICULO|ART\.?|Arto\.?|ARTO\.?)\s+((?:\d+[\s.]*(?:bis|ter)?|\d+[A-Z]?(?:\.\d+)?|[ÚU]NICO|[ÚU]nico|UNICO))\s*[.°º]*[-.:–]?\s*([^\n]*)/gimu,
];

const STRUCTURE_RE = /(?:^|\n)\s*((?:T[ÍI]TULO|TITULO|CAP[ÍI]TULO|CAPITULO|SECCI[ÓO]N|SECCION|DISPOSICION(?:ES)?(?:\s+(?:TRANSITORIAS?|FINALES?|DEROGATORIAS?|GENERALES?|COMPLEMENTARIAS?|COMUNES?))?)\s+[IVXLC0-9]+[^\n]*)/gimu;

const DISPOSICIONES_RE = /(?:^|\n)\s*(DISPOSICION(?:ES)?\s+(?:TRANSITORIAS?|FINALES?|DEROGATORIAS?|GENERALES?|COMPLEMENTARIAS?|COMUNES?))\s*$/gimu;
...
/**
 * Find the start of actual law text.
 * Nicaraguan laws typically begin with preamble from
 * "LA ASAMBLEA NACIONAL" or "EL PRESIDENTE DE LA REPUBLICA".
 */
function findLawTextStart(text: string): number {
  const startPatterns = [
    ...
    /\bDECRETA\s*:/i,
    /\bRESUELVE\s*:/i,
    /(?:^|\n)\s*(?:ART[ÍI]CULO|Art[íi]culo|ARTICULO|Arto\.?|ARTO\.?)\s+(?:1|PRIMERO|[ÚU]NICO|UNICO)\s*[.°º]*[-.:–]/im,
  ];
...
export function parseNILawText(text: string, act: ActIndexEntry): ParsedAct {
 ...
 */
export function parseNILawHtml(html: string, act: ActIndexEntry): ParsedAct {
  let bodyHtml = html;
  ...
  return parseNILawText(text, act);
}
